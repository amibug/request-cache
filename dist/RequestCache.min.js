(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["RequestCache"] = factory();
	else
		root["RequestCache"] = factory();
})(this, function() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _requestCache = __webpack_require__(1);
	
	var RequestCache = _interopRequireWildcard(_requestCache);
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.generateCacheKey = exports.removeCache = exports.getCache = exports.setCache = undefined;
	
	var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; };
	
	var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }(); //缓存的 key 里仅包括 url 和 params，这样是为了可以同时缓存不同 params 的结果
	...
	var _localstoreLru = __webpack_require__(4);
	
	var _localstoreLru2 = _interopRequireDefault(_localstoreLru);
	...
	var store = new _localstoreLru2.default();
	
	// HACK: 当 url 中有 disableCache=true 时，禁用 localStorage
	var DISABLE_CACHE = location.search.indexOf('disableCache=true') > -1;
	// HACK: 当 url 中有 showLog=true 时，显示命中日志
	var SHOW_LOG = location.search.indexOf('showLog=true') > -1;
	
	// 约定以下参数自动过滤掉
	var skipStoreParams = ['dtMaxAge', 'dtExpireTime', 'callback', 'sycmToken', 'ctoken', 'token', 't', '_', '_t'];
	
	// 存储的 key 名
	var DATA_KEY = '_data'; // 实际数据
	var EXPIRE_KEY = '_expireTime'; // 过期时间
	var DELETE_KEY = '_deleteTime'; // 过期后删除时间
	 ...
	 */
	var _generateCacheKey = function _generateCacheKey(url) {
		...
		if (url.indexOf('?') === -1) {
			...
			var _ret = function () {
				...
				var _url$split = url.split('?'),
				    ...
				    urlPart = _url$split2[0],
				    paramsPart = _url$split2[1];
				...
				paramsPart.split('&').forEach(function (paramsStr) {
					var _paramsStr$split = paramsStr.split('='),
					    _paramsStr$split2 = _slicedToArray(_paramsStr$split, 2),
					    k = _paramsStr$split2[0],
					    v = _paramsStr$split2[1];
	
					mergedParams[k] = v;
				});
				return {
					v: _generateCacheKey(urlPart, stripParams(mergedParams, skipStoreParams))
				};
			}();
	...
	function setCache(url) {
		...
		var identityKeyFunc = options.identityKeyFunc;
		// 禁用缓存或数据为空时不缓存
	
		if (DISABLE_CACHE || params.__disableCache || (0, _lodash.isEmpty)(data)) {
			return;
		}
		try {
			var _store$set;
	
			var expireTime = void 0,
			    deleteTime = void 0;
	
			expireTime = generateCacheExpire(params);
			deleteTime = expireTime + 12 * 12 * 60 * 60000;
	
			var key = generateCacheKey(url, params);
			store.set(key, (_store$set = {}, _defineProperty(_store$set, DATA_KEY, data), _defineProperty(_store$set, EXPIRE_KEY, expireTime), _defineProperty(_store$set, DELETE_KEY, deleteTime), _defineProperty(_store$set, IDENTITY_KEY, (0, _lodash.isFunction)(identityKeyFunc) ? identityKeyFunc(url, params) : ''), _store$set), true);
		} catch (e) {}
	}
	...
	function getCache(url) {
		...
		// 以下参数放在url中无效
		var _params$__forceToCach = params.__forceToCache,
		    __forceToCache = _params$__forceToCach === undefined ? false : _params$__forceToCach,
		    _params$__showLog = params.__showLog,
		    __showLog = _params$__showLog === undefined ? false : _params$__showLog,
		    _params$__disableCach = params.__disableCache,
		    __disableCache = _params$__disableCach === undefined ? false : _params$__disableCach;
		...
		try {
			var key = generateCacheKey(url, params);
			var storeData = store.get(key);
			var identityKey = (0, _lodash.isFunction)(identityKeyFunc) ? identityKeyFunc(url, params) : '';
	
			if (!storeData) return null;
	
			if (storeData && storeData[DATA_KEY]) {
				if (identityKey !== storeData[IDENTITY_KEY]) {
					// identityKey 变化，返回空,用于区分不同用户的缓存数据
					return null;
				}
				if (storeData[DELETE_KEY] < Date.now()) {
					// 检查是否需要删除
					store.remove(key);
					return null;
				}
				...
				if (storeData[EXPIRE_KEY] >= Date.now()) {
					// 检查是否过期
					if (SHOW_LOG || __showLog) {}
					return storeData[DATA_KEY];
				} else {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var StoregeLru = function () {
		function StoregeLru() {
			_classCallCheck(this, StoregeLru);
	  ...
	  */
		...
		_createClass(StoregeLru, [{
			...
			value: function setItem(key, val) {
				...
				storage.setItem(key, (0, _util.serialize)(val));
				return val;
			}
			...
			value: function set(key, value) {
				...
				try {
					...
					if (force) {
						...
						while (max > 0) {
							max--;
							var qkeys = this.getLruQueueKeys(this._sizePerTry);
							if (qkeys.length > 0) {
	   ...
	   */
			...
			key: 'setQueue',
			value: function setQueue(key) {
				var cacheQueue = this.getItem(this._cache_queue) || {};
				var current = cacheQueue[key];
				if (current == null) {
					cacheQueue[key] = {};
	   ...
	   */
			...
			key: 'removeQueue',
			value: function removeQueue(key) {
				var cacheQueue = this.getItem(this._cache_queue) || {};
			...
			key: 'getLruQueueKeys',
			value: function getLruQueueKeys(size) {
				var cacheQueue = this.getItem(this._cache_queue) || {};
				// 从高到低排序
				// 清理时我们根据频率的使用fre标志，fre最小的优先清理，同时相同的fre，我们优先清理time比较小的
				var keys = Object.keys(cacheQueue).sort(function (a, b) {
					var AQueue = cacheQueue[a];
					var BQueue = cacheQueue[b];
					return BQueue.fre == AQueue.fre ? BQueue.time - AQueue.time : BQueue.fre - AQueue.fre;
				});
	
				var ret = keys.length < size ? keys : keys.slice(keys.length - size);
				return ret;
			}
		}]);
...
/***/ function(module, exports) {
	...
	var endOfToday = function endOfToday() {
...
;
//# sourceMappingURL=RequestCache.min.js.map